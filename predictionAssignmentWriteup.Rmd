...
set.seed(33833)
randomForestModel1Fit <- 
    train(classe ~ ., data=cleanTrainingData, method="rf")
...
cleanTrainingData2 <- 
    cleanTrainingData[,colnames(cleanTrainingData) %in% 
                       c(variableImportanace$variable[1:25],"classe")]

set.seed(33833)
randomForestModel2Fit <- 
    train(classe ~ ., data=cleanTrainingData2, method="rf")
```

```{r randompForestModel3}
cleanTrainingData3 <- 
    cleanTrainingData[,colnames(cleanTrainingData) %in% 
                       c(variableImportanace$variable[1:14],"classe")]

set.seed(33833)
randomForestModel3Fit <- 
    train(classe ~ ., data=cleanTrainingData3, method="rf")
```

```{r generatePredictions}
testDataIn <- testData[,colnames(testData) %in% 
                        c(variableImportanace$variable[1:25])]
...
[10]: http://stackoverflow.com/questions/17604789/using-ggplot2-create-faceted-scatterplot-with-scaled-and-moved-density
[11]: http://stackoverflow.com/questions/22237783/user-defined-colour-palette-in-r-and-ggpairs